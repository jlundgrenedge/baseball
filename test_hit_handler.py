...
class TestHitHandler(unittest.TestCase):
    def test_handle_hit_baserunning_scores_run(self):
        # Setup
        ...
        # We need to patch the module-level function or the method if it's a method
        # HitHandler calls baserunning.decide_runner_advancement
        
        handler = HitHandler(field_layout, baserunning_sim)
        ...
        # We need to ensure decide_runner_advancement returns "home"
        # Since it's imported in hit_handler.py, we can patch it there
        # But for now, let's rely on the fact that a single usually scores from third
